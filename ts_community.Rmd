...
The script gets `DASH`, `SwapScore` and wallet `Address` from the googlesheet. It adds colums:
  ...
  + `Hwins`: The expected winning tickets one would receive if the total number of   launchpad slots in a NFT system based on `HybridScore` would be 1000 (one launchpad slot = one winning ticket).
...
## What happens if a DASH = 2000 would be sufficient to KYC for a launchpad project?
